...
def security():
    print(say_wish())
    print("""\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
             This software is for physical activity tracking purpose , 
             so we also keep you data privately accessible ,
             to ensure enought security , we will ask you for login details . 
             initially you need to signup first 
             ///////////////////////////////////////////////////////////
             """)
    db.execute("create table if not exists signup(username varchar(20),password varchar(20))")
    ...
    while True:
        ...
        print("""<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
              Warning for privacy reasons i format every saved data from the databases 
              when you are a existing user and  select signup again ,so that any other person cannot able to acces your data. 
              Be careful while entering your choice  
              >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
              """)
